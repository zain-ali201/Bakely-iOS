...
- (void)viewDidLoad
{
    ...
    if (get)
    {
        [self.imgWishList setImage:[UIImage imageNamed:@"prod-fav"]];
        [Util setPrimaryColorImageView:self.imgWishList];
    }
    else
    {
        [self.imgWishList setImage:[UIImage imageNamed:@"prod-fav-empty"]];
    }
...
-(void)setDefaultData
{
    ...
    if (htmlString3 != nil && ![htmlString3 isEqualToString:@""])
    {
...
-(void)setFrames {
    ...
    if (isSeller)
    {
        ...
        if ([[product.dict_seller_info valueForKey:@"store_tnc"] length] > 0)
        {
            ...
            {
                self.wvSellerDetails.frame = CGRectMake(8, self.btnSellerMoreDetail.frame.origin.y + self.btnSellerMoreDetail.frame.size.height + 2, self.view.frame.size.width - 32, 80);
            }
            else
            {
