...
//    shadowflag = true;
    ...
    if (get)
    {
        [self.btnWishList setImage:[UIImage imageNamed:@"detail-fav"] forState:UIControlStateNormal];
    }
    else
    {
        [self.btnWishList setImage:[UIImage imageNamed:@"detail-fav-empty"] forState:UIControlStateNormal];
    }
...
{
    self.scrlImages.frame = CGRectMake(16, 8, SCREEN_SIZE.width - 32, self.scrlImages.frame.size.height);
    ...
    for (int i = 0; i < arrImageCopy.count; i++)
    {
        @autoreleasepool
        {
            ...
            imageview.frame = CGRectMake(xPos, 0, SCREEN_SIZE.width - 32, self.scrlImages.frame.size.height);
...
//        self.lblInStock.textColor = [UIColor redColor];
    ...
    self.wvSellerDetails.userInteractionEnabled = NO;
...
//    self.lblDiscountRate.attributedText = [Util setPriceForItem:htmlString];
    self.lblDiscountRate.text = [NSString stringWithFormat:@"%@ %.1f", appDelegate.strCurrencyCode, product.price];
    self.lblDiscountRate.textColor = UIColor.whiteColor;
    
    NSString * htmlString4 = product.desc;
    NSMutableAttributedString * attrStr4 = [[NSMutableAttributedString alloc] initWithData:[htmlString4 dataUsingEncoding:NSUnicodeStringEncoding] options:@{ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType } documentAttributes:nil error:nil];
    self.lblDescription.attributedText = attrStr4;
...
//        self.lblInStock.frame = CGRectMake(self.lblAvailability.frame.origin.x + self.lblAvailability.frame.size.width + 8,self.lblAvailability.frame.origin.y, self.lblInStock.frame.size.width, self.lblAvailability.frame.size.height);
    ...
    if (product.additional_Info != nil && ![product.additional_Info isEqualToString:@""])
    {
        ...
        self.wvAttributes.userInteractionEnabled = NO;
...
//        self.vwInfo.frame = CGRectMake(0, self.vwName.frame.origin.y + self.vwName.frame.size.height + (15*SCREEN_SIZE.width/375), SCREEN_SIZE.width, self.vwInfo.frame.size.height);
        ...
        self.wvAttributes.frame = CGRectMake(8, 8, self.vwAttributes.frame.size.width - 16, self.wvAttributes.frame.size.height);

        CGRect frame = self.wvAttributes.frame;
        frame.size.height = 1;
        self.wvAttributes.frame = frame;
        CGSize fittingSize = [self.wvAttributes sizeThatFits:CGSizeZero];
        frame.size = fittingSize;
        self.wvAttributes.frame = frame;
        
        self.vwAttributes.frame = CGRectMake(8, self.vwInfo.frame.origin.y + self.vwInfo.frame.size.height - (15*SCREEN_SIZE.width/375), self.vwInfo.frame.size.width - 16, self.wvAttributes.frame.size.height + 16);
        self.vwInfo.hidden = false;
        self.vwAttributes.hidden = false;
    }
    else
...
//        self.lblSoldBy.font=Font_Size_Product_Name_Not_Bold;
        ...
        if ([[product.dict_seller_info valueForKey:@"store_tnc"] length] > 0)
        {
            ...
            CGSize fittingSize = [self.wvSellerDetails sizeThatFits:CGSizeZero];

            if (fittingSize.height>80)
            {
                    self.wvSellerDetails.frame = CGRectMake(8, self.btnSellerMoreDetail.frame.origin.y + self.btnSellerMoreDetail.frame.size.height + 2, self.view.frame.size.width - 32, 100);
            }
            else
            {
                          self.wvSellerDetails.frame = CGRectMake(8, self.btnSellerMoreDetail.frame.origin.y + self.btnSellerMoreDetail.frame.size.height + 2, self.view.frame.size.width - 32, fittingSize.height);
            }
...
//            self.btnSellerDescMore.titleLabel.font = Font_Size_Product_Name_Not_Bold;
            ...
            self.btnSellerDescMore.frame = CGRectMake(8, self.wvSellerDetails.frame.origin.y + self.wvSellerDetails.frame.size.height + 5, self.btnSellerDescMore.frame.size.width, self.btnSellerDescMore.frame.size.height);
            self.wvSellerDetails.hidden = NO;
            self.btnSellerDescMore.hidden = NO;

        }
        else
        {
            self.wvSellerDetails.frame = CGRectMake(8, self.btnSellerMoreDetail.frame.origin.y + self.btnSellerMoreDetail.frame.size.height, self.view.frame.size.width - 16, 0);
            self.btnSellerDescMore.frame = CGRectMake(8, self.wvSellerDetails.frame.origin.y + self.wvSellerDetails.frame.size.height, self.btnSellerDescMore.frame.size.width, 0);
            self.wvSellerDetails.hidden = YES;
    ...
    else
    {
        ...
        else
        {
            ...
            [UIView animateWithDuration:0.4f animations:^{
                ...
                self.btnWishList.transform = CGAffineTransformMakeScale(1.3, 1.3);
            } completion:^(BOOL finished){

                // for zoom out
                [UIView animateWithDuration:0.4f animations:^{

                    self.btnWishList.transform = CGAffineTransformMakeScale(1, 1);
                }];
            }];
