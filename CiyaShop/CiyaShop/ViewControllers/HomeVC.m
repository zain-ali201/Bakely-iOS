...
@interface HomeVC () <UITabBarDelegate, UITableViewDelegate, UITableViewDataSource, RedirectToMostPopularProductDelegate, RedirectToDealOfTheDayDelegate, SetCategoryDelegate, ClickOnRecentProductDelegate, UIGestureRecognizerDelegate, UIScrollViewDelegate>
...
{
    NSMutableArray *imagesArray, *arrInfiniteProductData, *arrTitle, *arrContent, *arrSubContent;
    ...
    BOOL successData, homeResponse, menuFlag;
...
-(IBAction)menuHomeBtnAction:(id)sender
{
    [UIView animateWithDuration:0.3
    animations:^{
        self->_leading.constant = 0;
        self->_trailing.constant = 0;
...
//        _menuView.alpha = 0;
    ...
    else
    {
        [UIView animateWithDuration:0.3
        animations:^{
            self->_leading.constant = 250;
            self->_trailing.constant = -250;
            [self.view layoutIfNeeded];
        }];
//        _menuView.alpha = 1;
        menuFlag = TRUE;
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self->_pageController.currentPage == 1)
        {
            [self->_imgBg setImage:[UIImage imageNamed:@"Home-1-bg"]];
            [self->_imgBg1 setImage:[UIImage imageNamed:@"Home-1-bg"]];
        }
        else if (self->_pageController.currentPage == 2)
        {
            [self->_imgBg setImage:[UIImage imageNamed:@"Home-3-bg"]];
            [self->_imgBg1 setImage:[UIImage imageNamed:@"Home-3-bg"]];
        }
        else
        {
            [self->_imgBg setImage:[UIImage imageNamed:@"Home-2-bg"]];
            [self->_imgBg1 setImage:[UIImage imageNamed:@"Home-2-bg"]];
        }
    });
}
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    ...
    return appDelegate.arrMainCategory.count;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    {
        cell.imgIcon.layer.cornerRadius = 10.0;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if(appDelegate.arrMainCategory.count > 5) {
        return CGSizeMake((SCREEN_SIZE.width/2)-17, ((SCREEN_SIZE.width/2)-10) + 12);
    }
    return CGSizeMake((SCREEN_SIZE.width/2)-17, ((SCREEN_SIZE.width/2)-10) + 12);
}
    ...
    if (success)
    {
        if (dictionary.count > 0)
        {
            ...
            if(!([dictionary valueForKey:@"all_categories"] == [NSNull null]))
            {
                ...
                NSLog(@"%i",appDelegate.arrCategory.count);
                NSLog(@"%i",arrTitle.count);
                [_tblMenu reloadData];
            }
            else
            {
            ...
            {
                appDelegate.strCurrencyCode = [[dictionary valueForKey:@"price_formate_options"] valueForKey:@"currency_code"];
